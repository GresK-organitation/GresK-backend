...
public class GetPublicRiderUseCase {

    private final RiderRepositoryPort riderRepository;

    @Transactional(readOnly = true)
    public TechnicalRider execute(String riderId, String shareToken) {
        TechnicalRider rider = riderRepository.findById(
                        com.gresk.modules.rider.domain.model.RiderId.of(riderId))
                .orElseThrow(() -> new RiderNotFoundException(riderId));

        if (rider.getShareToken() == null || !rider.getShareToken().equals(shareToken)) {
